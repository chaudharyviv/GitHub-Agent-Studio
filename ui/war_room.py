...
from ui.components import agent_status_dashboard, compare_view, memory_panel, render_events, report_viewer
...
def _run_team(store: MemoryStore, state: dict, owner: str, repo: str, chosen: list[str], focus: str, dashboard, live,
    ...
    for ev in stream_war_room(owner, repo, focus or None, store=store, only=chosen,
        ...
        if ev.kind == "error":
            ...
            current = live.status(f"{ev.title} specialist working…", expanded=True)
        elif ev.kind == "agent" and current is not None:
            with current:
                render_events([ev.event], live=True)
        elif ev.kind == "agent_done" and current is not None:
            status = next(s for s in ev.statuses if s.agent_id == ev.agent_id)
            current.update(label=f"{ev.title}: {status.findings} finding(s) · {ev.usage.split('·')[-1].strip()}",
                           state="complete" if status.state == "done" else "error", expanded=False)
        elif ev.kind == "manager_start":
            current = live.status("Manager writing the report…", expanded=False)
        elif ev.kind == "report":
            state["report"], state["narrative"] = ev.report.markdown, ev.report.narrative
            current.update(label="Manager: report ready", state="complete")
        elif ev.kind == "done":
...
def render_war_room(store: MemoryStore, owner: str, repo: str, repo_id: str, max_output_tokens: int, lite_mode: bool,
    ...
    with run_tab:
        ...
        st.caption("Specialists run one after another, then the Manager writes the report. "
