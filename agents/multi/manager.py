...
class ManagerAgent(Agent):
    ...
    def __init__(self, store: MemoryStore, client: Any = None, model: Optional[str] = None,
                 max_output_tokens: int = DEFAULT_MAX_OUTPUT_TOKENS, limits: Optional[Limits] = None,
    ...
    def synthesize(self, owner: str, repo: str, session_id: str, statuses: Sequence[AgentStatus], query: Optional[str] = None,
                    parallel: bool = False) -> HealthReport:
        ...
        report = HealthReport(
            repo_id=repo_id, session_id=session_id, narrative=narrative, execution_note=execution_note_text(parallel),
            statuses=list(statuses), findings=findings,
            unknown_refs=unknown_citations(narrative, findings), usage=self.usage.summary(), generated_at=generated_at, error=error,
            markdown=assemble_report(repo_id, statuses, findings, narrative, generated_at, parallel=parallel),
        )
        self.store.set_session_metadata(session_id, json.dumps({
            ...
            "statuses": [s.model_dump() for s in statuses], "execution_note": report.execution_note,
        }))
        return report
    ...
    def _write_narrative(self, repo_id: str, findings: Sequence[Finding], statuses: Sequence[AgentStatus], query: Optional[str]) -> tuple[str, Optional[str]]:
        ...
        try:
            ...
            response = provider.chat(
                [{"role": "system", "content": get_manager_prompt()},
                 {"role": "user", "content": build_manager_input(repo_id, findings, statuses, query, limits=self.limits)}],
                max_tokens=self.max_output_tokens, temperature=0.2,
            )
...
def build_manager_input(repo_id: str, findings: Sequence[Finding], statuses: Sequence[AgentStatus], query: Optional[str] = None,
    ...
    shown = sort_findings(findings)[:limits.max_findings_for_manager]
    lines.append(f"\nFindings ({len(shown)} of {len(findings)} shown), each as [#id] agent | severity/category | confidence: text | evidence:")
    for f in shown:
        evidence = evidence_text(f)[:limits.max_evidence_chars]
